...
type time_primitive =
  | TimeSpan of time_span
  | TimeStamp of time_stamp
...
type geometry =
  | Point of point
  | LineString of line_string
  | LinearRing of linear_ring
  | Polygon of polygon
...
type item_icon_state = Open | Closed | Error | Fetching0 | Fetching1 | Fetching2

type item_icon =
  { state : item_icon_state ;
    href : string }
...
type style =
  { id : object_id ;
    icon_style : icon_style option ;
    label_style : label_style option ;
    line_style : line_style option ;
    poly_style : poly_style option ;
    balloon_style : balloon_style option ;
    list_style : list_style option }
...
type network_link =
  { feature : feature_params ;
    ...
    fly_to_view : bool option ;
    link : link }

type feature =
  | Document of document
  | Folder of folder
  | NetworkLink of network_link
  | Placemark of placemark
  | GroundOverlay of ground_overlay
  | PhotoOverlay of photo_overlay
  | ScreenOverlay of screen_overlay
...
type update =
  { changes : (update_operation * klm_data) array }
...
type kml =
  { hint : string option ;
    ...
    network_link_control : network_link_control option }
